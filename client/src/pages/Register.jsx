...
export default function Register() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "User already exists or registration failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageTransition>
      <div className="min-h-screen flex items-center justify-center bg-slate-50/80 dark:bg-slate-950/80 px-6 py-12 transition-colors duration-200 relative z-10">
        <AnimatedBackground />
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] bg-lime-500/10 dark:from-lime-500/5 blur-3xl rounded-full -z-10"></div>
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-md border border-slate-200/50 dark:border-slate-800/50 p-8 md:p-10 rounded-2xl shadow-xl w-full max-w-md space-y-6">
          <div className="text-center">
            <Link to="/" className="inline-flex items-center space-x-2 font-display text-2xl font-bold bg-linear-to-r from-teal-600 to-lime-600 dark:from-teal-400 dark:to-lime-400 bg-clip-text text-transparent mb-3 font-display">
              SHRINK
            </Link>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  {...register("name")}
                  className={`w-full pl-11 pr-4 py-3 rounded-xl border bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/25 ${errors.name ? 'border-red-500' : 'border-slate-200/70 dark:border-slate-800/70'}`}
                />
              </div>
              {errors.name && (
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-11 pr-11 py-3 rounded-xl border bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/25 ${errors.confirmPassword ? 'border-red-500' : 'border-slate-200/70 dark:border-slate-800/70'}`}
                />
              </div>
